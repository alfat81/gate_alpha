...
class GateTestnetClient:
    ...
    async def create_order(
        ...
        try:
            ...
            logger.info("Creating order", currency_pair=currency_pair, side=side, amount=amount, price=price, type=order_type)
            order = spot_api.create_order(order_params)
            logger.info("Order created", order_id=order.id, status=order.status)
            return order
        except ApiException as ae:
            logger.error("Gate API error creating order", status=ae.status, reason=ae.reason, currency_pair=currency_pair)
            raise
    ...
    async def cancel_order(self, order_id: str, currency_pair: str) -> bool:
        ...
        try:
            ...
            logger.error("Gate API error cancelling order", status=ae.status, reason=ae.reason, order_id=order_id)
            raise
    ...
    async def get_order(self, order_id: str, currency_pair: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("Gate API error fetching order", status=ae.status, reason=ae.reason, order_id=order_id)
            raise
