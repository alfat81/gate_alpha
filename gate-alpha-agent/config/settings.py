...
class TradingConfig(BaseSettings):
    ...
    # Scanner settings - relaxed for Alpha pairs detection
    min_volume_usd: float = Field(default=50000.0, gt=0.0, description="Minimum 24h volume in USD")
    max_volume_usd: float = Field(default=10000000.0, gt=0.0, description="Maximum 24h volume in USD")
    min_change_24h_pct: float = Field(default=5.0, gt=0.0, description="Minimum |24h change| percentage")
    max_spread_pct: float = Field(default=5.0, gt=0.0, le=100.0, description="Maximum bid-ask spread percentage")
